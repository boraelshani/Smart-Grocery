...
# Instead of blindly matching, check if we accidentally deleted the closing `</div>` for combo deal.
import re

# The bug: "Encountered unknown tag 'endif'. Jinja was looking for 'endblock'."
# This means an `{% endif %}` has no matching `{% if %}` at the block level.
# OR an `{% endfor %}` is missing.

# Let's inspect the `{% if %}` around line ~988 to ~1018.

correct_replacement = r'''<div class="card-body d-flex flex-column h-100" style="background-color: #fff;">
              ...
              <div class="mt-auto pt-3">
                ...
                <div class="d-flex flex-column gap-2">
                  ...
                  <button type="button" class="btn-soft compare-secondary-btn btn-add-compare"
                    data-product-id="{{ p.get('id') }}"
                    data-product-name="{{ p.get('name') or p.get('title','')|e }}"
                    data-product-price="{{ (p.get('cheapest') and p.get('cheapest').get('price')) or p.get('price','') }}"
                    data-product-unit-price="{{ p.get('normalized_unit_price') if p.get('normalized_unit_price') is not none else '' }}"
                    data-product-unit-label="{{ p.get('normalized_unit_label','') }}"
                    data-product-confidence="{{ confidence.get('label', '') }}"
                    data-product-image="{{ prefer_processed(p.get('image') or (p.get('images') and p.get('images')[0]) or url_for('static', filename='placeholder.svg'))|e }}"
                    onclick="event.stopPropagation();">
                    <i class="bi bi-scales icon"></i> Compare
                  </button>
                </div>
              </div>
            </div>
...
if start_str not in text:
    start_str = r'<div class="card-body d-flex flex-column h-100" style="background-color: #fff;">'
    
if start_str in text:
    # Safely isolate the entire product card logic
    pattern = re.compile(start_str + r'.*?\{% endfor %\}', re.DOTALL)
    text, count = pattern.subn(correct_replacement, text)
