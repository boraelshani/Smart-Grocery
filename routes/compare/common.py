...
def compare_best_basket():
    try:
        ...
        if not isinstance(product_ids, list) or not product_ids: return jsonify({'error': 'product_ids required'}), 400
        store_meta = build_store_meta_map(stores_model.list_stores())
        products = []
        for pid in product_ids[:30]:
            doc = _fetch_product_or_deal_by_id(str(pid))
            if doc: products.append(build_compare_product_payload(helpers.sanitize_mongo_doc(doc), store_meta_map=store_meta))
        mixed_total = 0.0; mixed_breakdown = []; single_store_totals = {}
        for prod in products:
            ...
            mixed_breakdown.append({'product_id': prod.get('id'), 'product_name': prod.get('name'), 'store': best.get('store'), 'price': best.get('price')})
            for s in stores:
                name = s.get('store')
        ...
        full_coverage = [{'store': name, 'total': val['total']} for name, val in single_store_totals.items() if val['coverage'] == len(products)]
