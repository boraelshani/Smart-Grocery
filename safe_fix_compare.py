...
# 1. Safely remove the Modal without destroying the bottom of the page.
modal_str = r'<div class="modal fade" id="priceReportModal" tabindex="-1" aria-labelledby="priceReportLabel" aria-hidden="true">.*?<!-- JavaScript Dependencies -->'
pattern = re.compile(modal_str, re.DOTALL)
text, count1 = pattern.subn('<!-- JavaScript Dependencies -->', text)
print(f"Removed modal: {count1}")

# 2. Fix the product card inner HTML layout.
start_str = r'<div class="card-body d-flex flex-column h-100"'
end_str = r'\{% endfor %\}'
